...
function query_post_type($query) {
  if(is_category() || is_tag() || is_home() && empty( $query->query_vars['suppress_filters'] ) ) {
	...
	if($post_type)
	    ...
	    $post_type = array('post','trip','classroom', 'news', 'opportunities', 'resource', 'schools', 'nav_menu_item');
...
function quark_scripts_styles() {
	...
	// Register and enqueue our site font
	
	wp_register_style( 'brix', trailingslashit( get_template_directory_uri() ) . 'css/MyFontsWebfontsKit.css' , array( 'normalize' ), '4.2.0', 'all' );
	wp_enqueue_style( 'brix' );
	...
	// Google Maps scripts
if (is_page('service-area')) {
	wp_register_script( 'google-map', 'https://maps.googleapis.com/maps/api/js?v=3.exp&key=AIzaSyCWX2-Jde3XIZZgiNtTm7xgyv3WTngy_Is', array(), '3', true );
	wp_enqueue_script( 'google-map' );
	wp_register_script( 'google-map-cluster', 'http://google-maps-utility-library-v3.googlecode.com/svn/trunk/markerclustererplus/src/markerclusterer.js', array('google-map'), '3', true );
	wp_enqueue_script( 'google-map-cluster' );
	wp_register_script( 'google-map-init', trailingslashit( get_template_directory_uri() ) . 'js/acfmap.js', array('google-map', 'jquery', 'google-map-cluster'), '0.1', true );
	...
	function quark_content_nav( $nav_id ) {
		...
		<nav role="navigation" id="<?php echo $nav_id; ?>" class="<?php echo $nav_class; ?>">
			...
			<?php if ( is_single() ) { // navigation links for single posts ?>
				...
				<?php previous_post_link( '<div class="nav-previous">Previous: %link &nbsp;/&nbsp;</div>', '<span class="meta-nav">' . _x( '', 'Previous post link', 'quark' ) . '</span> %title' ); ?>
				<?php next_post_link( '<div class="nav-next"> Next: %link</div>', ' %title <span class="meta-nav">' . _x( '', 'Next post link', 'quark' ) . '</span>' ); ?>
...
function quark_continue_reading_link() {
	return '&hellip;<p><a class="more-link" href="'. esc_url( get_permalink() ) . '" title="' . esc_html__( 'Continue reading', 'quark' ) . ' &lsquo;' . get_the_title() . '&rsquo;">' . wp_kses( __( 'Keep reading <span class="meta-nav">&rarr;</span>', 'quark' ), array( 'span' => array( 
			'class' => array() ) ) ) . '</a></p>';
}
...
function quark_theme_options_styles() {
	...
	*/
	$footerColour = apply_filters( 'of_sanitize_color', of_get_option( 'footer_color', '#ffffff' ) );
